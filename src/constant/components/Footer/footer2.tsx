...
<div class="bg-white pt-4 sm:pt-10 lg:pt-12">
  <footer class="mx-auto max-w-screen-2xl px-4 md:px-8">
    <div style='flex-direction:column' class="flex flex-col items-center justify-between gap-4 border-t border-b py-6 md:flex-row">
      <nav class="flex flex-wrap justify-center gap-x-4 gap-y-2 md:justify-start md:gap-6">
         {{#each links}}
          <a href="#" class="text-gray-500 transition duration-100 hover:text-indigo-500 active:text-indigo-600">{{name}}</a>
          {{/each}}
      </nav>
    ...
    <div class="py-8 text-center text-sm text-gray-400">{{tagline}}</div>
  </footer>
</div>
`;
